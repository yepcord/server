...
from ..models.interactions import InteractionCreate, InteractionRespond
...
from ...yepcord.models import User, Application, ApplicationCommand, Integration, Message
...
async def create_interaction(user: User):
    ...
    await getGw().dispatch(InteractionCreateEvent(interaction, True), users=[application.id])
...
async def respond_to_interaction(data: InteractionRespond, interaction: Interaction):
    if data.type == InteractionCallbackType.CHANNEL_MESSAGE_WITH_SOURCE:
        ...
        message = await Message.objects.create(id=Snowflake.makeId(), author=bot_user, content=d.content, flags=flags,
                                               interaction=interaction, channel=interaction.channel, ephemeral=True)
        ...
        if not is_ephemeral:
            await getGw().dispatch(MessageCreateEvent(message_obj), channel_id=interaction.channel)
