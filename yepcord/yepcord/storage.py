...
class FClient(Client):
    ...
    async def s_upload(self, path: str, data: bytes | BytesIO) -> None:
        dirs = path.split("/")[:-1]
        for dir in dirs:
...
class _Storage(metaclass=SingletonABCMeta):
    ...
    @abstractmethod
    async def _read(self, path: str) -> bytes | None: ...  # pragma: no cover
    ...
    async def getAvatar(self, uid: int, avatar_hash: str, size: int, fmt: str) -> bytes | None:
    ...
    async def getChannelIcon(self, cid: int, icon_hash: str, size: int, fmt: str) -> bytes | None:
        anim = icon_hash.startswith("a_")
        def_size = 256 if anim else 1024
        return await self._getImage("channel_icon", cid, icon_hash, size, fmt, def_size, lambda s: s)

    async def getGuildIcon(self, gid: int, icon_hash: str, size: int, fmt: str) -> bytes | None:
        anim = icon_hash.startswith("a_")
        def_size = 256 if anim else 1024
        return await self._getImage("icon", gid, icon_hash, size, fmt, def_size, lambda s: s)

    async def getGuildAvatar(self, uid: int, gid: int, avatar_hash: str, size: int, fmt: str) -> bytes | None:
        anim = avatar_hash.startswith("a_")
        def_size = 256 if anim else 1024
        return await self._getImage(f"guild/{gid}/avatar", uid, avatar_hash, size, fmt, def_size, lambda s: s)

    async def getSticker(self, sticker_id: int, size: int, fmt: str, animated: bool) -> bytes | None:
    ...
    async def getGuildEvent(self, event_id: int, event_hash: str, size: int, fmt: str) -> bytes | None:
        return await self._getImage("guild_event", event_id, event_hash, size, fmt, 600, lambda s: int(9 * s / 16))

    async def getEmoji(self, emoji_id: int, size: int, fmt: str, anim: bool) -> bytes | None:
        def_fmt = "gif" if anim else "png"
    ...
    async def getRoleIcon(self, rid: int, icon_hash: str, size: int, fmt: str) -> bytes | None:
        anim = icon_hash.startswith("a_")
        def_size = 256 if anim else 1024
        return await self._getImage("role_icon", rid, icon_hash, size, fmt, def_size, lambda s: s)

    async def getBanner(self, uid: int, banner_hash: str, size: int, fmt: str) -> bytes | None:
    ...
    async def getGuildSplash(self, gid: int, banner_hash: str, size: int, fmt: str) -> bytes | None:
        anim = banner_hash.startswith("a_")
        def_size = 480 if anim else 600
        return await self._getImage("splash", gid, banner_hash, size, fmt, def_size, lambda s: int(9 * s / 16))

    async def getAppIcon(self, aid: int, icon_hash: str, size: int, fmt: str) -> bytes | None:
        anim = icon_hash.startswith("a_")
        def_size = 256 if anim else 1024
    ...
    async def getAttachment(self, channel_id: int, attachment_id: int, name: str) -> bytes | None:
